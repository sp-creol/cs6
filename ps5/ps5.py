...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        phraseWords = str.split(self.phrase)
        c = 0
        builtWord = ""
        for i in range(len(text)+1):
            if i == len(text) or text[i] in string.punctuation or text[i] == " ":
                if len(builtWord) > 0:
                    if str.lower(builtWord) == str.lower(phraseWords[c]):
                        c += 1
                        if c == len(phraseWords):
                            return True
                    else:
                        c = 0
                    builtWord = ""
    ...
    for line in lines:
        ...
        for i in range(len(line) + 1):
            if i == len(line) or line[i] == ",":
                formedTerms.append(formedString)
                formedString = ""
            else:
                formedString += line[i]
        dividedLines.append(formedTerms)
    ...
    for line in dividedLines:
        if line[0] == "ADD":
            for i in range(1, len(line)):
                returnList.append(triggers[line[i]])
        else:
            if line[1] == "TITLE":
                ...
                triggers[line[0]] = AndTrigger(triggers[line[2]],triggers[line[3]])
            elif line[1] == "OR":
                triggers[line[0]] = OrTrigger(triggers[line[2]],triggers[line[3]])
    return returnList
